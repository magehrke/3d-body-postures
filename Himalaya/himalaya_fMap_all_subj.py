# -*- coding: utf-8 -*-
"""
Created on Mon May 23 09:46:50 2022
...
subject_folder = os.path.join(data_folder, 'Main_effect/newdata')
...
# subj = ["S5","S6","S7","S8","S9","S10","S11","S13","S14","S15","S16","S17","S18","S19","S20","S21","S22","S23","S25","S26"]
subj = ["S13", "S14", "S15", "S16"]
# subj = ["S5"]
# feature_names = ["kp2d","kp3d","gabor"]
feature_names = ["kp2d", "kp3d", "gabor", "S_it_output", 'VAEparam', 'VAE_enc', 'VAE_dec']
...
# roi = ["_EBA_RH_RFX_20subj","_EBA_LH_RFX_20subj"]

# roi = ["_FBA_RH","_FBA_LH"]

# mod_ind = np.asarray([[0,1],[0,2]])
# mod_ind = np.asarray([[1,2]])
# mod_ind = np.asarray([[0,6],[1,6],[2,6],[3,6],[4,6],[5,6],[0,1],[0,2],[1,2],[3,4],[3,5],[4,5],[2,5]])
data3 = mat73.loadmat(model_folder + '/hrf.mat')
hrf = np.asarray(data3['hrf'])
data5 = mat73.loadmat(model_folder + '/training_runs_onsets.mat')
...
for s in subj:
    ...
    file_data = os.path.join(subject_folder, string_data)
    print(string_data)
    ...
    for f in range(3):
        ...
        for feature_space in feature_names:
            ...
            file_name = os.path.join(model_folder, f"{feature_space}"f"{mod_name}_cv"f"{f + 1}_standard.h5")
        ...
        cv = generate_leave_one_run_out(n_samples_train, run_onset[f, :])
        ...
        # n_targets_batch = 200
        # n_alphas_batch = 5
        # n_targets_batch_rrfit = 200
        n_targets_batch = 2000
        n_alphas_batch = None
        n_targets_batch_refit = None  # TODO: !!
        ...
        solver_params = dict(max_iter=200, hyper_gradient_method="direct",
                             max_iter_inner_hyper=1,
                             initial_deltas="here_will_go_the_previous_deltas", n_targets_batch=n_targets_batch,
                             tol=1e-5)
        ...
        deltas.append(backend.to_numpy(pipe_2[-1].deltas_))
    ...
    scipy.io.savemat(output, {'r2_scores': cv_scores, 'r2_scores_split': cv_scores_split, 'corr_scores': corr_tot,
                              'corr_scores_split': corr_scores_split, 'deltas': deltas})
