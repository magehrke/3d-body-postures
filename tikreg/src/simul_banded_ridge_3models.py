...
subj = ["S5", "S6", "S7", "S8", "S9", "S10", "S11", "S13", "S15", "S16", "S17", "S18", "S19", "S20", "S22", "S23"]
model = ["kp2d", "kp3d", "gabor"]
# model = ["kp3d","kp2d","gabor"]
# model = ["VAE_enc_L1","VAE_enc_L2","VAE_enc","VAE_dec_L1","VAE_dec_L2","VAE_dec","VAEparam"]

mod_ind = np.asarray([[0, 1, 2]])
...
for s in subj:
    ...
    for mod_i in range(mod_ind.shape[0]):
        ...
        file_mod3 = data_folder / (model[mod_ind[mod_i][2]] + mod_suffix)
        data1 = mat73.loadmat(file_mod1)
        data2 = mat73.loadmat(file_mod2)
        data3 = mat73.loadmat(file_mod3)
        print_and_log(f'Iteration {mod_i+1}: Using kp2d, kp3d & gabor')
        ...
        corr3 = np.zeros((3, nvox))
        beta1, beta2, beta3, pred1, pred2, pred3, hyperparam = [], [], [], [], [], [], []
        ...
        for f in range(3):
            ...
            Xtrain_cv_mod3 = np.asarray(data3['Xtrain'][f]).squeeze()
            Xtest_cv_mod3 = np.asarray(data3['Xtest'][f]).squeeze()
            ...
            mean3 = mat.repmat(np.mean(Xtrain_cv_mod3, axis=0), ntest, 1)
            std3 = mat.repmat(np.std(Xtrain_cv_mod3, axis=0), ntest, 1)

            X1tr = stats.zscore(Xtrain_cv_mod1)
            X2tr = stats.zscore(Xtrain_cv_mod2)
            X3tr = stats.zscore(Xtrain_cv_mod3)
            X1te = (Xtest_cv_mod1 - mean1) / std1
            X2te = (Xtest_cv_mod2 - mean2) / std2
            X3te = (Xtest_cv_mod3 - mean3) / std3
            ...
            X3tr_hrf = np.zeros(X3tr.shape)
            X1te_hrf = np.zeros(X1te.shape)
            X2te_hrf = np.zeros(X2te.shape)
            X3te_hrf = np.zeros(X3te.shape)
            ...
            for i in range(X3tr.shape[1]):
                X3tr_hrf[:, i] = np.convolve(X3tr[:, i], hrf)[0:ntrain]
            ...
            for i in range(X3te.shape[1]):
                X3te_hrf[:, i] = np.convolve(X3te[:, i], hrf)[0:ntest]
            ...
            feat_priors3 = spatial_priors.SphericalPrior(p3, hyparams=[1.0])
            alphas = np.logspace(0, 4, 11)
            fit_banded_polar = models.estimate_stem_wmvnp([X1tr_hrf, X2tr_hrf, X3tr_hrf], Ytr,
                                                          [X1te_hrf, X2te_hrf, X3te_hrf], Yte,
                                                          feature_priors=[feat_priors1, feat_priors2, feat_priors3],
            ...
            lambda_threes = fit_banded_polar['optima'][:, 3]
            ...
            w3 = np.linalg.multi_dot([X3tr_hrf.T, kernel_weights, np.diag(alphas), np.diag(lambda_threes ** -2)])

            # Check that we can recover predictions for separate models
            # predictions = pred model1 + pred model2 + pred model3
            Yhat = np.matmul(X1te_hrf, w1) + np.matmul(X2te_hrf, w2) + np.matmul(X3te_hrf, w3)
            for i in range(nvox):
            ...
            Yhat3 = np.matmul(X3te_hrf, w3)
            for i in range(nvox):
                Yhat3[:, i] = Yhat3[:, i] / alphas[i]
            ...
            pred2.append(Yhat2)
            pred3.append(Yhat3)
            hyperparam.append([[lambda_ones], [lambda_twos], [lambda_threes], [alphas]])
            ...
            for i in range(Yhat.shape[1]):
                ...
                corr3[f, i] = np.corrcoef(Yhat3[:, i], Yte[:, i])[0, 1]
        ...
        mean_perf_3 = np.mean(corr3, axis=0)

        out_s = s + "_results_body_" + model[mod_ind[mod_i][0]] + "_" + model[mod_ind[mod_i][1]] \
            + "_" + model[mod_ind[mod_i][2]] + ".mat"
        output = data_folder / out_s

        scipy.io.savemat(output, {'mean_perf': mean_perf, 'mean_perf_1': mean_perf_1, 'mean_perf_2': mean_perf_2,
                                  'mean_perf_3': mean_perf_3, 'pred1': pred1, 'pred2': pred2, 'pred3': pred3})
        # Skip saving betas and hyperparams for now
        # ,'beta1': beta1, 'beta2': beta2, 'hyperparams': hyperparam})
